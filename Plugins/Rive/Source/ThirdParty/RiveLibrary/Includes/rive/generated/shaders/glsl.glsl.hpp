...
// clang-format off
#if defined(_EXPORTED_RENDER_MODE_MSAA) && defined(_EXPORTED_ENABLE_CLIP_RECT) && defined(GL_ES)
// clang-format on
...
#else
#define TEXTURE_GATHER(NAME, SAMPLER_NAME, COORD, TEXTURE_INVERSE_SIZE)         \
    make_half4(TEXEL_FETCH(NAME, int2(COORD) + int2(-1, 0)).x,                 \
               TEXEL_FETCH(NAME, int2(COORD) + int2(0, 0)).x,                  \
               TEXEL_FETCH(NAME, int2(COORD) + int2(0, -1)).x,                 \
               TEXEL_FETCH(NAME, int2(COORD) + int2(-1, -1)).x)
#endif
...
// clang-format off
#if _EXPORTED_GLSL_VERSION >= 310 && defined(_EXPORTED_VERTEX) && defined(_EXPORTED_RENDER_MODE_MSAA) && defined(_EXPORTED_ENABLE_CLIP_RECT)
out gl_PerVertex
{
